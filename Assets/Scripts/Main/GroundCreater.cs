...
public class GroundCreater : MonoBehaviour
{
	...
	void init()
	{
		groundChipWidth = GroundChip.transform.localScale.x / 100;
		groundChipHeight = GroundChip.transform.localScale.y / 100;

		wPos = First_WPos;
		//wPos = First_WPos + (groundChipWidth * 70);
		wNum = Pre_Create_Num;
		currentWNum = 0;
		stage = 0;
		prevSign = 1;
	...
	IEnumerator stage0()
	{
		for (var i = 0; i < 20; ++i) {
			...
			for (var h = 0; h < Height_Num; ++h) {
				...
				hPos += groundChipHeight;

				go.GetComponent<SpriteRenderer>().color = Color.red;

				Main.progress();
			}
			wPos += groundChipWidth;
			if (i >= 19) {
				Debug.Log("hoge");
				++stage;
				stage1();
	...
	void stage1()
	{
		...
		//normalStage(Color.red);
		StartCoroutine(normalStage(Color.blue));
	}
	...
	void stage2()
	{
		...
		StartCoroutine(normalStage(Color.white));
	}
	...
	/// ただ平坦なステージ
	/// </summary>
	IEnumerator normalStage(Color32 col)
	{
		for (; currentWNum < wNum; ++currentWNum) {
			if (wNum >= Width_Num) {
				stageTransition();
